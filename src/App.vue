...
<form class="grid gap-4 grid-rows-4 grid-cols-3">
  ...
  <div class="col-start-2 row-start-1 col-end-4 inline-grid gap-2 grid-cols-2 ">
    ...
    <input class="col-start-2" step=0.01 placeholder="min" type="number"  id="cantidadAfiliados" v-model="form['cuotaMax']">
    <input class="col-start-2" step=0.01 placeholder="max" type="number" id="cantidadAfiliados" v-model="form['cuotaMin']">
  </div>
...
const handleRandomsMinMax = () => {
  ...
  if(sum < total){
    tabla.value[tabla.value.length-1].cuota =tabla.value[tabla.value.length-1].cuota + Math.round((total - sum) * 100) / 100
    sum =total
  }
  totalSum.value = sum
